...
class StaticApiKeyAuthenticator implements AuthenticationInterface {
    ...
    {
        if (Config::get('auth.static_api_key.location') == 'header')
        {
            $request->addHeader(Config::get('auth.static_api_key.field'), Config::get('auth.static_api_key.value'));
        }
        else
        {
            ...
            $request->body(array_merge($payload,
                [Config::get('auth.static_api_key.field') => Config::get('auth.static_api_key.value')]));
        }
    }
}
